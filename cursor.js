(function(){
  try{
    ...
    style.textContent =
      ...
      '#dc-cursor{width:20px;height:20px;margin:-10px 0 0 -10px;border-radius:50%;border:1.5px solid #a97c25;' +
        'background:radial-gradient(circle, rgba(169,124,37,.22), rgba(169,124,37,0) 70%);' +
        'transition:transform .12s cubic-bezier(.2,.9,.3,1), opacity .25s ease, border-color .2s ease, background .2s ease;}' +
      '#dc-cursor.dc-click{transform:scale(1.7) translate(0,0);border-color:#2e6b58;}' +
      '#dc-nun{font-family:"Amiri",serif;font-size:19px;line-height:1;color:#a97c25;' +
        'text-shadow:0 0 6px rgba(169,124,37,.35);transition:opacity .35s ease;}' +
    ...
    function spawnInk(x, y){
      ...
      dot.style.cssText = 'position:fixed;left:' + x + 'px;top:' + y + 'px;width:7px;height:7px;margin:-3.5px 0 0 -3.5px;' +
        'border-radius:50%;background:#a97c25;pointer-events:none;z-index:2147483646;' +
