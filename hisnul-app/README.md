...
- **Prayer-time reminder notifications** (`reminders.js` in the web app) use
  the plain Web Notification / Service Worker API. That's unreliable for
  background notifications inside a native WebView. To get real OS
  notifications, add `@capacitor/local-notifications` and have
  `reminders.js` call it (via `window.Capacitor`) when running inside the
  native app, falling back to the existing web API when running as the PWA.
